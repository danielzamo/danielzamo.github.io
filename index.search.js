var relearn_search_index = [
  ...
  {
    "content": " Despliegue de servidor host anfitrión host virtualización KVM. Despliegue de cluster Ubuntu server LTS k8s minikube (driver kvm).\nTecnologías utilizadas minikube KVM Ubuntu LTS 22.04.. Especificación inicial …\nDependecias cumplida … … Actualización sistema, instalando paquetes (opcionales) dzamo@ubuntu-k8s:~$ sudo apt -y update \u0026\u0026 sudo apt -y upgrade Instalación/configuración de minikube En este apartado se instala y configura un cluster minikube con driver kvm (REF LINK).\n# Instalacion minikube wget https://storage.googleapis.com/minikube/releases/latest/minikube-linux-amd64 sudo install minikube-linux-amd64 /usr/local/bin/minikube minikube version Instalación de kubectl sudo curl -LO https://storage.googleapis.com/kubernetes-release/release/`curl \\ -s https://storage.googleapis.com/kubernetes-release/release/stable.txt`/bin/linux/amd64/kubectl sudo chmod 755 ./kubectl sudo mv kubectl /usr/local/bin/ kubectl version -o json --client ",
